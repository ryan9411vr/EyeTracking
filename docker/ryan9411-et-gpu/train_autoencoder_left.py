...
# -*- coding: utf-8 -*-
"""
Left-eye conditional auto-encoder
...
def load_rows_from_db(db_path: str) -> List[Tuple[str, float, float]]:
    """Return (leftEyeFrame, theta1, theta2) rows only."""
    sql = f"""
...
# DATA GENERATOR
# ──────────────────────────────────────────────────────────────────────────────
class LeftEyeSequence(Sequence):
    ...
    def __getitem__(self, idx: int):
        ...
        imgs, lbls = [], []
        for i in batch_idx:
            l_enc, theta1, theta2 = self.rows[i]
...
# ──────────────────────────────────────────────────────────────────────────────
def main():
    ap = argparse.ArgumentParser(description="Train left-eye conditional auto-encoder")
    ...
    train_gen = LeftEyeSequence(train_rows, BATCH_SIZE, augment=True, shuffle=True)
    val_gen   = LeftEyeSequence(val_rows,   BATCH_SIZE, augment=False, shuffle=False)
    ...
    enc.save(os.path.join(args.output_dir, "encoder_left.h5"))
    dec.save(os.path.join(args.output_dir, "decoder_left.h5"))
