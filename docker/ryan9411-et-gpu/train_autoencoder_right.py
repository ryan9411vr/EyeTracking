...
# -*- coding: utf-8 -*-
"""
Right-eye conditional auto-encoder
...
# ──────────────────────────────────────────────────────────────────────────────
BATCH_SIZE    = 64
IMAGE_SIZE    = 128          # single eye 128 × 128
MAX_OFFSET    = 10
MAX_ROTATION  = 15
...
def load_rows_from_db(db_path: str) -> List[Tuple[str, float, float]]:
    """Return (rightEyeFrame, theta1, theta2) rows only."""
    sql = f"""
...
# DATA GENERATOR
# ──────────────────────────────────────────────────────────────────────────────
class RightEyeSequence(Sequence):
    ...
    def __getitem__(self, idx: int):
        ...
        imgs, lbls = [], []
        for i in batch_idx:
            r_enc, theta1, theta2 = self.rows[i]
...
def build_models():
    ...
    # full latent vector (5 sup | 3 unsup)
    latent_vec = layers.Concatenate(name="latent_concat")([sup_pred, unsup_latent])

    # ── Decoder (single-eye; upsample to 128×128) ─────────────────────────
    dec_in = Input(shape=(8,), name="decoder_input")
    d = layers.Dense(32 * 32 * 128, activation="relu")(dec_in)
    d = layers.Reshape((32, 32, 128))(d)
...
# ──────────────────────────────────────────────────────────────────────────────
def main():
    ap = argparse.ArgumentParser(description="Train right-eye conditional auto-encoder")
    ...
    train_gen = RightEyeSequence(train_rows, BATCH_SIZE, augment=True, shuffle=True)
    val_gen   = RightEyeSequence(val_rows,   BATCH_SIZE, augment=False, shuffle=False)
    ...
    enc.save(os.path.join(args.output_dir, "encoder_right.h5"))
    dec.save(os.path.join(args.output_dir, "decoder_right.h5"))
