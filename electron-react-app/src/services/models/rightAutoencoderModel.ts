/**
 * Right-eye conditional auto-encoder ▸ ENCODER-ONLY
...
import { updateLatentsRight } from '../../slices/statusSlice';

let encoder: tf.LayersModel | null = null;
const ENCODER_PATH = 'right_autoencoder/model.json'; // relative to modelFolderPath

/** Load right-eye encoder (TF-JS bundle) */
export async function loadModel(modelFolderPath: string): Promise<void> {
  try {
    ...
    console.log('RightAutoencoder ▸ encoder loaded from', url);
  } catch (err) {
    console.error('RightAutoencoder ▸ failed to load encoder:', err);
    encoder = null;
  }
}
 ...
 */
export function encodeLatents(right: tf.Tensor3D): number[] | null {
  if (!encoder) {
    // console.warn('RightAutoencoder ▸ encoder not loaded.');
    return null;
  }

  const latent = tf.tidy(() => {
    const batched = right.expandDims(0); // 1 × 128 × 128 × 3
